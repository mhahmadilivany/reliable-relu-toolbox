...
#import copy
from rrelu.setup import build_data_loader, replace_act  #build_model, replace_act_all
...
if __name__ == "__main__":
    ...
    data_loader_dict, n_classes = dataset_utils.load_dataset(args.dataset, args.batch_size)
    ...
    print(args.dataset, args.model, args.name_relu_bound, args.name_serach_bound, 
          args.bounds_type, args.bitflip, args.iterations)
    ...
    if args.bitflip=='fixed':
        with torch.no_grad():
            for name, param in model.named_parameters():
                if param!=None:
                    param.copy_(torch.tensor(Fxp(param.clone().cpu().numpy(), True, 
                                                 n_word=args.n_word, n_frac=args.n_frac, 
                                                 n_int=args.n_int).get_val()))
    ...
    if args.name_relu_bound=="none":
        for fault_rate in args.fault_rates:
            val_results_fault = eval_fault(model, data_loader_dict, fault_rate,args.iterations, 
                                           args.bitflip, args.n_word, args.n_frac, args.n_int)
        ...
        model = replace_act(model, args.name_relu_bound, args.name_serach_bound, 
                            data_loader_dict, args.bounds_type, args.bitflip)
