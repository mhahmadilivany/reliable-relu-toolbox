...
import random
from pytorchfi.core_train import FaultInjection
...
def train(
    ...
    for iteration in range(2):
        ...
        for fault in fault_rates:
            ...
            if bitflip=='float':
                pfi_model.original_model = multi_weight_inj_float(pfi_model,fault)
            elif bitflip=='fixed':    
                pfi_model.original_model = multi_weight_inj_fixed(pfi_model,fault)
            elif bitflip =="int":
                pfi_model.original_model = multi_weight_inj_int (pfi_model,fault)  
            ...
            for epoch in range(
                ...
                val_info_dict = eval(pfi_model.original_model, data_provider)
# print(val_info_dict)
    return model
    ...
    with tqdm(
        ...
        for _, (images, labels) in enumerate(data_provider['train']):
            ...
            with torch.no_grad():
                ...
                teacher_output = pfi_model.corrupted_model(images).detach()
                teacher_logits = F.softmax(teacher_output, dim=1)
            s_feats, s_out = pfi_model.original_model.module.extract_feature(images)
            ...
            for i in range(feat_num):
                ...
                # loss_distill.sum()
            optimizer.zero_grad()
            nat_logits = pfi_model.original_model(images)
            kd_loss = cross_entropy_loss_with_soft_target(
                        nat_logits,teacher_logits
                    )
            loss =   loss_distill # criterion(nat_logits, labels)  #+ kd_loss +   #+ torch.mean(kd_loss) +   #+ 0.1 * loss_distill + 0.9 *
            loss.backward()
            # for name, par in pfi_model.original_model.named_parameters():
